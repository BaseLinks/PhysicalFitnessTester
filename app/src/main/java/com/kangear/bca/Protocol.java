...
import com.kangear.bodycompositionanalyzer.IProtocol;
import com.kangear.bodycompositionanalyzer.WelcomeActivity;
...
public class Protocol implements IProtocol {
    ...
    // 二.通信格式
    // [引导符3][消息长度1] [填充位3][数 据n][填充位2][结束符1][校验和1]
    public static final int MSG_HEAD_LENGTH         = 0x03; // 引导符长度
    public static final int MSG_LENGTH_LENGTH       = 0x01; // 消息长度
    public static final int MSG_START_OTHRE_LENGTH  = 0x03; // 开始填充
    public static final int MSG_DATA_LENGTH         = 0x00; // n 不可用
    public static final int MSG_END_OTHRE_LENGTH    = 0x02; // 结束填充
    public static final int MSG_END_TAG_LENGTH      = 0x01; // 结束符
    public static final int MSG_CRC_LENGTH          = 0x01; // 校验和

    public static final int MSG_HEAD_START         = 0x00;
    public static final int MSG_LENGTH_START       = MSG_HEAD_START + MSG_HEAD_LENGTH;
    public static final int MSG_START_OTHRE_START  = MSG_LENGTH_START + MSG_LENGTH_LENGTH;
    public static final int MSG_DATA_START         = MSG_START_OTHRE_START + MSG_START_OTHRE_LENGTH;
    public static final int MSG_END_OTHRE_START    = MSG_DATA_START + MSG_DATA_LENGTH;
    public static final int MSG_END_TAG_START      = MSG_END_OTHRE_START + MSG_END_OTHRE_LENGTH;
    public static final int MSG_CRC_START          = MSG_END_TAG_START + MSG_CRC_LENGTH; // n
    ...
    public static final byte[] RECV_HEAD = {(byte) 0xCC, (byte) 0x00, (byte) 0x00};
    public static final byte[] RECV_START_OTHER_DATA = {(byte) 0xDE, (byte) 0x00, (byte) 0xFE};
    public static final byte[] END_TAG = {(byte) 0xDD};
...
//    public static final int dataLength = 0;
    ...
    private static int getMsgLength(int pureDataLength) {
        Log.i(TAG, "pureDataLength: " + pureDataLength);
//        public static final int MSG_HEAD_LENGTH         = 0x03; // 引导符长度
//        public static final int MSG_LENGTH_LENGTH       = 0x01; // 消息长度
//        public static final int MSG_START_OTHRE_LENGTH  = 0x03; // 开始填充
//        public static final int MSG_DATA_LENGTH         = 0x00; // n 不可用
...
//        public static final int MSG_END_TAG_LENGTH      = 0x01; // 结束符
//        public static final int MSG_CRC_LENGTH          = 0x01; // 校验和
        return MSG_START_OTHRE_LENGTH + pureDataLength + MSG_END_OTHRE_LENGTH + MSG_END_TAG_LENGTH + MSG_CRC_LENGTH;
    }

    private static int getCrcEnd(int msgLength) {
//        public static final int MSG_HEAD_LENGTH         = 0x03; // 引导符长度
//        public static final int MSG_LENGTH_LENGTH       = 0x01; // 消息长度
//        public static final int MSG_START_OTHRE_LENGTH  = 0x03; // 开始填充
//        public static final int MSG_DATA_LENGTH         = 0x00; // n 不可用
...
//        public static final int MSG_END_TAG_LENGTH      = 0x01; // 结束符
//        public static final int MSG_CRC_LENGTH          = 0x01; // 校验和
        return MSG_HEAD_LENGTH + MSG_LENGTH_LENGTH + msgLength - MSG_CRC_LENGTH;
    }

    private static int getTotalLength(int msgLength) {
        return MSG_HEAD_LENGTH + MSG_LENGTH_LENGTH + msgLength;
    }

    private static int getPureDataLength(int dataLength) {
//        public static final int MSG_HEAD_LENGTH         = 0x03; // 引导符长度
//        public static final int MSG_LENGTH_LENGTH       = 0x01; // 消息长度
//        public static final int MSG_START_OTHRE_LENGTH  = 0x03; // 开始填充
//        public static final int MSG_DATA_LENGTH         = 0x00; // n 不可用
...
//        public static final int MSG_END_TAG_LENGTH      = 0x01; // 结束符
//        public static final int MSG_CRC_LENGTH          = 0x01; // 校验和
        return dataLength - (MSG_START_OTHRE_LENGTH + MSG_END_OTHRE_LENGTH + MSG_END_TAG_LENGTH + MSG_CRC_LENGTH);
    }
...
//        return byteArray;
    ...
    public static byte[] createResponse(byte[] data) {
        ...
        int pureDataLength = tmpDataArray.length;
        // 消息长度 指 [消息长度1] [填充位3][数 据n]
        int msgLength = getMsgLength(pureDataLength);
        int BYTE_BUFFER_ALLOCATE = 1024;
        byte[] byteArray = null;
        ByteBuffer target = ByteBuffer.allocate(BYTE_BUFFER_ALLOCATE);
        target.put(RECV_HEAD); // CC 00 00
        target.put((byte)(msgLength & 0xFF)); // ?E1
        target.put(RECV_START_OTHER_DATA); // DE 00 FE
        target.put(tmpDataArray); // DATA
        target.put(new byte[]{0x00, (byte)0xDD}); // 填充数据
        target.put(END_TAG); // 结束符
        int crcStart = 0x01;
        int crcEnd = getCrcEnd(msgLength);
        target.put(calcCRC(Arrays.copyOfRange(target.array(), crcStart, crcEnd)));
    ...
    private static final byte[] tmpdata = {
            ...
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x28, (byte)0xA4, 0x06, 0x20, 0x03, 0x63, 0x19, (byte)0xB8, 0x15, 0x4D, 0x00, 0x7F, 0x0A,
            0x76, 0x0A, 0x79, 0x01, (byte)0xE4, 0x09, (byte)0xA3, 0x09, (byte)0x8F, 0x14, (byte)0xB6, 0x12, 0x00, 0x00, 0x00, 0x00, 0x00, 0x7B, 0x02, 0x16,
            0x02, (byte)0xB5, 0x02, 0x35, 0x02, (byte)0xFC, 0x01, 0x1B, 0x02, (byte)0xEB, 0x00, 0x5F, 0x00, 0x7F, 0x00, 0x12, 0x02, (byte)0xD5, 0x01, (byte)0xF5,
            0x01, 0x36, 0x01, 0x0A, 0x01, 0x4A, 0x01, (byte)0x9C, 0x01, 0x6D, 0x01, (byte)0x84, 0x01, 0x76, 0x00, 0x64, 0x00, 0x71, 0x00, (byte)0x23,
            0x24, 0x26, 0x00, (byte)0x85, 0x00, (byte)0x90, 0x00, (byte)0xA4, 0x00, 0x17, 0x01, (byte)0xF7, 0x00, 0x0B, 0x01, 0x26, 0x1F, 0x21, 0x26, 0x1F,
            0x21, 0x1C, 0x01, (byte)0xEA, 0x00, (byte)0xF9, 0x00, 0x54, 0x00, 0x56, 0x00, 0x5C, 0x00, 0x56, 0x00, 0x56, 0x00, 0x5C, 0x00, 0x0E,
            0x06, 0x08, 0x0E, 0x06, 0x08, (byte)0x88, 0x00, 0x2F, 0x00, 0x3F, 0x00, 0x24, 0x11, 0x17, 0x23, 0x11, 0x17, (byte)0xEB, 0x04, (byte)0x84,
            0x03, 0x4C, 0x04, 0x14, 0x01, (byte)0xB9, 0x00, (byte)0xF0, 0x00, 0x25, 0x01, (byte)0x96, 0x00, (byte)0xC8, 0x00, 0x44, 0x04, 0x46, 0x04, 0x53,
            0x04, (byte)0xB1, 0x03, 0x5D, 0x50, 0x5A, 0x20, 0x1E, 0x23, 0x03, 0x04, (byte)0xEC, 0x04, (byte)0xAA, 0x02, 0x2B, (byte)0xA5, (byte)0x80, 0x7C, (byte)0x80,
            0x2D, (byte)0x80, 0x0F, 0x07, (byte)0xDC, 0x0A};
...
//
        // 6. Package Data
        // 消息长度 指 [命令/状态][项目代码][项目地址][数据][CRC] 的总长度(字节数)
        int pureDataLength = getPureDataLength(length);
        start = MSG_DATA_START;
        end = start + pureDataLength;
        if (msg.length < end) {
            Log.e(TAG, "parsePackage: response.length error");
            throw new ProtocalExcption("parsePackage: response.length error");
        }
        ...
        // 7. end tag
        // total length
        tmpInt = getTotalLength(length);
        int endTagStart = tmpInt - (MSG_END_TAG_LENGTH + MSG_CRC_LENGTH);
        start = endTagStart;
        end = start + MSG_END_TAG_LENGTH;
        if (msg.length < end) {
            Log.e(TAG, "parsePackage: response.length error");
            throw new ProtocalExcption("parsePackage: response.length error");
        }
        b = Arrays.copyOfRange(msg, start, end);
        if (!Arrays.equals(b, END_TAG)) {
            Log.e(TAG, "parsePackage: RESPONSE_PACK_HEAD error");
            throw new ProtocalExcption("parsePackage: RESPONSE_PACK_HEAD error");
        }
        ...
        int startSum = 1;
        int endSum = getCrcEnd(msgLength);
        start = getTotalLength(msgLength) - MSG_CRC_LENGTH;
        ...
        Log.e(TAG, "parsePackage: " + bytesToHex(qr.getData()));
        return qr;
    }
    ...
    public synchronized byte[] connectData(byte[] b) {
        ...
        int totalLength = getTotalLength(length);
    ...
    public static byte[] calcCRC(byte[] buf) {
        Log.w(TAG, "calcCRC: " + bytesToHex(buf));
        byte crc = checkSum(buf);
        return new byte[]{crc};
    }
