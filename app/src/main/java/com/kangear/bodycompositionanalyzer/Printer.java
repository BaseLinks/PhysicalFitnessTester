...
public class Printer {
    ...
    public enum PrinterModel {
        ...
        XEROX_PHASER_3020("", "XEROX_PHASER_3020", DEVICE_ID_XEROX_PHASER_3020, "HelloWorld.epsonr330");
...
//        context.unregisterReceiver(mUsbPermissionActionReceiver);
     ...
     * @return 如果转换成功，返回true否则false
     */
    public static boolean covertPdfToXerox3020(String rasterPath, String pdf) {
        Log.i(LOG_TAG, "covertPdfToXerox3020");
        boolean ret = false;
        ShellUtils.CommandResult cr;
        if (ShellUtils.checkRootPermission()) {
            String cmd = "export PPD=/etc/cups/ppd/Phaser-3020.ppd && " +
                    "gs " +
                    "-dQUIET " +
                    "-dPARANOIDSAFER " +
                    "-dNOPAUSE " +
                    ...
                    "-c '<</.HWMargins[12.500000 12.500000 12.500000 12.500000] /Margins[0 0]>>setpagedevice' " +
                    "-f " + pdf + " " +
                    "| rastertospl 1 \"\" \"\" \"\" \"\" > "  +
                    rasterPath;

            Log.e(LOG_TAG, "covertPdfToXerox3020: str:" + cmd);
            cr = ShellUtils.execCommand(cmd, true);
            if(cr.result == 0) {
                cr = ShellUtils.execCommand("chmod 777 " + rasterPath, true);
                if(cr.result == 0)
...
//                    "-sColourModel=CMYK " +
    ...
    private static boolean covertPdfToPCL(String rasterPath, String pdf) {
        ...
        } else if (pm.equals(PrinterModel.XEROX_PHASER_3020)) {
